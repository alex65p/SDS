...
public class SchedeInterventoDPIBean extends BMPEntityBean implements ISchedeInterventoDPI, ISchedeInterventoDPIHome {
    ...
    private static SchedeInterventoDPIBean ys = null;

    private SchedeInterventoDPIBean() {
        //
    }

    public static SchedeInterventoDPIBean getInstance() {
        if (ys == null) {
            ys = new SchedeInterventoDPIBean();
        }
        return ys;
    }
...
//<comment description="setter/getters">
    ...
    public void setTPL_INR_DPI(String newTPL_INR_DPI) {
        if (TPL_INR_DPI.equals(newTPL_INR_DPI)) {
            return;
        }
...
/////////////////////<Kushkarov for new Search>///////////////////////////////////
    ...
    public Collection ejbfindEx(
            ...
            String ATI_INR, String NOM_RSP_INR,
            String PBM_RSC,
            long iOrderBy) {
        int i = 2;
        int indexLotti = 0;
        ...
        try {
            ...
            ps.setLong(1, COD_AZL);
            if (indexLotti != 0) {
                ps.setLong(indexLotti, COD_LOT_DPI);
            }
            if (indexDatPianif != 0) {
                ps.setDate(indexDatPianif, DAT_PIF_INR);
            }
            if (indexInr != 0) {
                ps.setDate(indexInr, DAT_INR);
            }
            if (indexAti != 0) {
                ps.setString(indexAti, ATI_INR.toUpperCase() + "%");
            }
            if (indexResp != 0) {
                ps.setString(indexResp, NOM_RSP_INR + "%");
            }
            if (indexPbm != 0) {
                ps.setString(indexPbm, PBM_RSC.toUpperCase() + "%");
            }
            ResultSet rs = ps.executeQuery();
            java.util.ArrayList al = new java.util.ArrayList();
            while (rs.next()) {
