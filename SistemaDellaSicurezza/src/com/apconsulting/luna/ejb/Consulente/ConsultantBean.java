...
////////////////////// CONSTRUCTOR///////////////////
    private static ConsultantBean ys = null;

    private ConsultantBean() {
        //
    }

    public static ConsultantBean getInstance() {
        if (ys == null) {
            ys = new ConsultantBean();
        }
        return ys;
    }
////////////////////////////ATTENTION!!//////////////////////////////////////////////////////////////////////////////
...
//----------------------------------------------------------
    ...
    public void ejbRemove() {
        ...
        bmp.beginTrans();
        try {
            // Elimino le opzioni del consulente
            ...
            if (ps.executeUpdate() == 0) {
                throw new NoSuchEntityException("ConsulENTE con ID=" + COD_COU + " non è stato trovato");
            }
...
//----------------------------------------------------------
    ...
    public void ejbStore() {
        ...
        try {
            ...
            if (ps.executeUpdate() == 0) {
                throw new NoSuchEntityException("Consultant con ID= non è trovata");
            }
        } catch (Exception ex) {
            ex.printStackTrace();
            throw new EJBException(ex);
...
//<comment description="setter/getters">
    ...
    public void removeCOD_AZL(String newCOD_COU, String newCOD_AZL) {
        ...
        try {
            ...
            if (ps.executeUpdate() == 0) {
                throw new NoSuchEntityException("Azienda con ID=" + newCOD_AZL + " non &egrave trovata");
            }
        } catch (Exception ex) {
            ex.printStackTrace();
            throw new EJBException(ex);
    ...
    public Collection ejbFindEx(String strNOM_COU, String strUSD_COU, String strRUO_COU, String strSTA_COU, java.sql.Date dDAT_ATT, java.sql.Date dDAT_DIS, int iOrderParameter) {
        ...
        if (strNOM_COU != null) {
            strSql += " AND UPPER(nom_cou) LIKE ? ";
        }
        if (strUSD_COU != null) {
            strSql += " AND UPPER(usd_cou) LIKE ? ";
        }
        if (strRUO_COU != null) {
            strSql += " AND UPPER(ruo_cou) LIKE ? ";
        }
        if (dDAT_ATT != null) {
            strSql += " AND dat_att = ? ";
        }
        ...
        try {
            ...
            if (strNOM_COU != null) {
                ps.setString(i++, strNOM_COU.toUpperCase());
            }
            if (strUSD_COU != null) {
                ps.setString(i++, strUSD_COU.toUpperCase());
            }
            if (strRUO_COU != null) {
                ps.setString(i++, strRUO_COU.toUpperCase());
            }
            if (dDAT_ATT != null) {
                ps.setDate(i++, dDAT_ATT);
            }
            if (dDAT_DIS != null) {
                ps.setDate(i++, dDAT_DIS);
            }
