...
package com.apconsulting.luna.ejb.Fascicolo;
 ...
 * @author Alessandro
 */
public class FascicoloBean extends BMPEntityBean implements IFascicolo, IFascicoloHome{
...
//< /member-varibles>
    ...
    private FascicoloBean() {
        //
    }

    private static FascicoloBean ys = null;

    public static FascicoloBean getInstance() {
        if (ys == null) {
            ys = new FascicoloBean();
        }
        return ys;
    }

    public void remove(Object primaryKey){
            FascicoloBean bean=new FascicoloBean();
            try{
    ...
    public IFascicolo create(String strOGG, String strCOD, String strREV,java.sql.Date dtDAT_EMI,java.sql.Date dtDAT_PRO, String strDOC_COL, long lCOD_PSC,long lCOD_PRO, long lCOD_AZL) throws javax.ejb.CreateException {
         FascicoloBean bean=new FascicoloBean();
             try{
    ...
    public IFascicolo findByPrimaryKey(Long primaryKey) throws javax.ejb.FinderException{
      FascicoloBean bean=new FascicoloBean();
			try{
					bean.setEntityContext(new EntityContextWrapper(primaryKey));
					bean.ejbActivate();
      ...
      public String getUltimaRevisione(long lCOD_PRO) {
        return (new FascicoloBean()).ejbUltimaRevisione(lCOD_PRO);
    }

      public String getDataProtocollo(long lCOD_PRO) {
        return (new FascicoloBean()).ejbDataProtocollo(lCOD_PRO);
    }

      public String getDataProtocolloPREC(long lCOD_PRO) {
        return (new FascicoloBean()).ejbDataProtocolloPREC(lCOD_PRO);
    }

      public String getUltimo_Doc_Sez(long lCOD_PRO) {
        return (new FascicoloBean()).ejbUltimo_Doc_Sez(lCOD_PRO);
    }

       public Collection getFascicoloPSCByID_View(long lCOD_PSC,long lCOD_PRO,long lCOD_AZL) {
        return (new FascicoloBean()).ejbGetFascicoloByID_View(lCOD_PSC,lCOD_PRO,lCOD_AZL);
    }
       
       public Collection getFascicoloPSCByID_View_SINTETICA(long lCOD_PSC, long lCOD_PRO) {
        return (new FascicoloBean()).ejbGetFascicoloByID_View_SINTETICA(lCOD_PSC,lCOD_PRO);
    }
       ...
       public Long ejbCreate(String strOGG, String strCOD, String strREV,java.sql.Date dtDAT_EMI,java.sql.Date dtDAT_PRO,String strDOC_COL,long lCOD_PSC,long lCOD_PRO, long lCOD_AZL)
  {
	...
	try{
		...
		PreparedStatement ps=bmp.prepareStatement("INSERT INTO psc_fsc_ope_tab (cod_fsc_ope,cod_psc,cod_pro,cod_azl,rev,cod,ogg,dat_emi,dat_pro,doc_col) VALUES(?,?,?,?,?,?,?,?,?,?)");
			ps.setLong(1, lCOD_FAS);
  ...
  public Collection ejbFindAll()
  {
      BMPConnection bmp=getConnection();
      try{
          PreparedStatement ps=bmp.prepareStatement("SELECT cod_fsc_ope FROM pcs_fsc_tab");
  ...
  public void ejbActivate(){
    this.lCOD_FAS=((Long)this.getEntityKey()).longValue();
  }
  ...
  public void ejbLoad(){
      BMPConnection bmp=getConnection();
      try{
         PreparedStatement ps=bmp.prepareStatement("SELECT cod_fsc_ope,cod,ogg,rev,dat_emi,dat_pro,doc_col,cod_psc FROM psc_fsc_ope_tab WHERE cod_fsc_ope=?");
           ps.setLong(1, lCOD_FAS);
           ResultSet rs=ps.executeQuery();
           if(rs.next()){
       		lCOD_FAS=rs.getLong(1);
			...
			lCOD_PSC=rs.getLong(8);
              ...
              throw new NoSuchEntityException("Testimone with ID="+lCOD_FAS+" not found");
           }
      }
      catch(Exception ex){
          throw new EJBException(ex);
  ...
  public void ejbRemove(){
      BMPConnection bmp=getConnection();
      try{
         PreparedStatement ps=bmp.prepareStatement("DELETE FROM psc_fsc_ope_tab WHERE cod_fsc_ope=?");
         ps.setLong(1, lCOD_FAS);
         if(ps.executeUpdate()==0) throw new NoSuchEntityException("Sezione generale with ID="+lCOD_FAS+" not found");
      }
      catch(Exception ex){
          throw new EJBException(ex);
      }
  ...
  public void ejbStore()
  {
    if(!isModified()) return;
      BMPConnection bmp=getConnection();
      try{
         PreparedStatement ps=bmp.prepareStatement("UPDATE psc_fsc_ope_tab SET cod=?, ogg=?, rev=?, dat_emi=?, dat_pro=?, doc_col=? WHERE cod_fsc_ope=?");
			...
			ps.setString(6, strDOC_COL);
                        ps.setLong(7, lCOD_FAS);

         if(ps.executeUpdate()==0) throw new NoSuchEntityException("Testimone with ID="+lCOD_FAS+" not found");
      }
      catch(Exception ex){
          throw new EJBException(ex);
      }
    ...
    public long getlCOD_FAS() {
        return lCOD_FAS;
    }

    public void setlCOD_FAS(long newlCOD_FAS) {
        if (lCOD_FAS == newlCOD_FAS) {
            return;
        }
        lCOD_FAS = newlCOD_FAS;
        setModified();
    }
    ...
    public String ejbUltimaRevisione (long lCOD_PRO) {
        ...
        BMPConnection bmp = getConnection();
        try {
            PreparedStatement ps = bmp.prepareStatement("SELECT MAX(REV) FROM psc_fsc_ope_tab WHERE cod_pro=?");
    ...
    public String ejbDataProtocollo (long lCOD_PRO) {
        ...
        BMPConnection bmp = getConnection();
        try {
            PreparedStatement ps = bmp.prepareStatement("select dat_pro from psc_fsc_ope_tab where rev=(SELECT MAX(rev) FROM psc_fsc_ope_tab WHERE cod_pro=?)AND COD_PRO=?");
            ps.setLong(1, lCOD_PRO);
    ...
    public String ejbDataProtocolloPREC(long lCOD_PRO) {
        ...
        try {
            PreparedStatement ps = bmp.prepareStatement(""
                    + "select dat_pro from psc_fsc_ope_tab "
                    + "where rev=(SELECT MAX(rev) FROM psc_fsc_ope_tab "
                    + "WHERE cod_pro=? and dat_pro=(select MAX (dat_pro)"
                    + " from psc_fsc_ope_tab where cod_pro=?))AND COD_PRO=?");
            ps.setLong(1, lCOD_PRO);
            ps.setLong(2, lCOD_PRO);
            ps.setLong(3, lCOD_PRO);
            rs = ps.executeQuery();
    ...
    public Collection ejbGetFascicoloByID_View(long lCOD_PSC,long lCOD_PRO,long lCOD_AZL) {
        BMPConnection bmp = getConnection();
        try {
            PreparedStatement ps = bmp.prepareStatement("SELECT 'Fascicolo dell''opera', a.ogg, (CASE WHEN a.rev = '0' THEN 'In Lav.' WHEN a.rev = '1' THEN 'Emiss.' ELSE a.rev END) AS rev, a.dat_emi, a.dat_pro, a.doc_col FROM psc_fsc_ope_tab a, (SELECT cod_psc, cod_pro, cod_azl, cod, MAX(rev) as rev FROM psc_fsc_ope_tab group by cod_psc, cod_pro, cod_azl, cod) b WHERE a.cod_psc=b.cod_psc and a.cod_pro=b.cod_pro and a.cod_azl=b.cod_azl and a.cod=b.cod and a.cod_psc = ? and a.cod_pro = ? and a.cod_azl = ? and not(a.rev = 0 and b.rev > 0) order by  a.cod, a.rev ");
    ...
    public Collection ejbGetFascicoloByID_View_SINTETICA(long lCOD_PSC, long lCOD_PRO) {
         BMPConnection bmp = getConnection();
        try {
            PreparedStatement ps = bmp.prepareStatement("SELECT psc_fsc_ope_tab.ogg, "
                    + "psc_fsc_ope_tab.rev, "
                    + "psc_fsc_ope_tab.dat_emi, "
                    + "psc_fsc_ope_tab.dat_pro, "
                    + "psc_fsc_ope_tab.doc_col "
                    + "FROM  psc_fsc_ope_tab "
                    + "WHERE cod_psc=? "
                    + "and rev=(SELECT MAX(rev) FROM psc_fsc_ope_tab "
                    + "WHERE cod_pro=? )"
                    + "order by psc_fsc_ope_tab.dat_emi,psc_fsc_ope_tab.rev");
            ps.setLong(1, lCOD_PSC);
            ps.setLong(2, lCOD_PRO);
