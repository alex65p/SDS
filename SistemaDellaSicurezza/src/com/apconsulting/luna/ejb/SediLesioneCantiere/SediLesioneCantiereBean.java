...
public class SediLesioneCantiereBean extends BMPEntityBean implements ISediLesioneCantiere, ISediLesioneCantiereHome {
    ...
    //</comment>
////////////////////// CONSTRUCTOR///////////////////
    private static SediLesioneCantiereBean ys = null;

    private SediLesioneCantiereBean() {
        //
    }

    public static SediLesioneCantiereBean getInstance() {
        if (ys == null) {
            ys = new SediLesioneCantiereBean();
        }
        return ys;
    }
...
//<comment description="Zdes opredeliayutsia metody EJB objecta kotorie budut rabotat v JSP Containere (Zaglushki)"/>
    ...
    public ISediLesioneCantiere create(String strNOM_SED_LES, String strTPL_SED_LES) throws CreateException {
        SediLesioneCantiereBean bean = new SediLesioneCantiereBean();
        try {
    ...
    public void remove(Object primaryKey) {
        SediLesioneCantiereBean iSediLesioneBean = new SediLesioneCantiereBean();
        try {
    ...
    public Collection getANA_SED_LES_TAB_ByNOM_View() {
        return (new SediLesioneCantiereBean()).ejbGetANA_SED_LES_TAB_ByNOM_View();
    }

    public Collection findEx(String NOM, long iOrderBy) {
        return (new SediLesioneCantiereBean()).ejbfindEx(NOM, iOrderBy);
    }
...
//<comment description="Zdes opredeliayutsia metody home intefeisa kotorie budut rabotat v EJB Containere"/>
    ...
    public Long ejbCreate(String strNOM_SED_LES, String strTPL_SED_LES) {
        ...
        try {
            PreparedStatement ps = bmp.prepareStatement("INSERT INTO ana_sed_les_can_tab (cod_sed_les,nom_sed_les,tpl_sed_les) VALUES(?,?,?)");
...
//--------------------------------------------------
    ...
    public Collection ejbFindAll() {
        BMPConnection bmp = getConnection();
        try {
            PreparedStatement ps = bmp.prepareStatement("SELECT cod_sed_les FROM ana_sed_les_can_tab ");
...
//----------------------------------------------------------
    ...
    public void ejbLoad() {
        BMPConnection bmp = getConnection();
        try {
            PreparedStatement ps = bmp.prepareStatement("SELECT * FROM ana_sed_les_can_tab  WHERE cod_sed_les=?");
            ps.setLong(1, COD_SED_LES);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
...
//----------------------------------------------------------
    ...
    public void ejbRemove() {
        BMPConnection bmp = getConnection();
        try {
            PreparedStatement ps = bmp.prepareStatement("DELETE FROM ana_sed_les_can_tab  WHERE cod_sed_les=?");
            ps.setLong(1, COD_SED_LES);
...
//----------------------------------------------------------
    ...
    public void ejbStore() {
        ...
        BMPConnection bmp = getConnection();
        try {
            PreparedStatement ps = bmp.prepareStatement("UPDATE ana_sed_les_can_tab  SET nom_sed_les=?, tpl_sed_les=? WHERE cod_sed_les=?");
...
// Views by Pogrebnoy Yura
    public Collection ejbGetANA_SED_LES_TAB_ByNOM_View() {
        BMPConnection bmp = getConnection();
        try {
            PreparedStatement ps = bmp.prepareStatement("SELECT cod_sed_les, nom_sed_les, tpl_sed_les FROM ana_sed_les_can_tab ORDER BY nom_sed_les");
            ResultSet rs = ps.executeQuery();
            java.util.ArrayList al = new java.util.ArrayList();
            while (rs.next()) {
                ANA_SED_LES_CAN_TAB_ByNOM_View obj = new ANA_SED_LES_CAN_TAB_ByNOM_View();
    ...
    public Collection ejbfindEx(String NOM, long iOrderBy) {
        String Sql = "SELECT cod_sed_les, nom_sed_les, tpl_sed_les FROM ana_sed_les_can_tab ";
        if (NOM != null) {
        ...
        try {
            ...
            java.util.ArrayList al = new java.util.ArrayList();
            while (rs.next()) {
                findEx_sed_les_can obj = new findEx_sed_les_can();
                obj.COD_SED_LES = rs.getLong(1);
                obj.NOM_SED_LES = rs.getString(2);
                obj.TPL_SED_LES = rs.getString(3);
                al.add(obj);
