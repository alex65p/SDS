 ...
 * @author Dario
 */
public class RischioCantiereBean extends BMPEntityBean implements IRischioCantiere, IRischioCantiereHome {
    ...
    private static RischioCantiereBean ys = null;

    private RischioCantiereBean() {
    }

    public static RischioCantiereBean getInstance() {
        if (ys == null) {
            ys = new RischioCantiereBean();
        }
        return ys;
    }
    ...
    public void remove(Object primaryKey) {
        RischioCantiereBean bean = new RischioCantiereBean();
        try {
    ...
    public IRischioCantiere create(
    ...
    public IRischioCantiere findByPrimaryKey(RischioPK primaryKey) throws javax.ejb.FinderException {
        RischioCantiereBean bean = new RischioCantiereBean();
        try {
            bean.setEntityContext(new EntityContextWrapper(primaryKey));
            bean.ejbActivate();
...
//---------------------
    ...
    public Collection ejbFindAll(long lAzienda) {
        ...
        try {
            PreparedStatement ps = bmp.prepareStatement("SELECT cod_rso FROM ana_rso_can_tab WHERE cod_azl=? ");
    ...
    public void ejbLoad() {
        BMPConnection bmp = getConnection();
        try {
            PreparedStatement ps = bmp.prepareStatement("SELECT cod_azl,cod_rso,nom_rso,des_rso,clf_rso,cod_fat_rso FROM ana_rso_can_tab WHERE cod_rso=?  AND cod_azl=? ");
            ...
            if (rs.next()) {
                ...
                strCLF_RSO = rs.getString(5);
                lCOD_FAT_RSO = rs.getLong(6);
            } else {
    ...
    public void ejbRemove(java.util.ArrayList alAziende) {
        BMPConnection bmp = getConnection();
        try {
            PreparedStatement ps = bmp.prepareStatement("DELETE FROM ana_rso_can_tab WHERE cod_rso=? AND ( cod_azl=? OR cod_azl IN (" + toString(alAziende) + ") )");
            ps.setLong(1, lCOD_RSO);
            ps.setLong(2, lCOD_AZL);
    ...
    public boolean isMultiple() {
        BMPConnection bmp = getConnection();
        try {
            PreparedStatement ps = bmp.prepareStatement("SELECT COUNT(*) FROM ana_rso_can_tab WHERE cod_rso=? ");
            ps.setLong(1, lCOD_RSO);
            ResultSet rs = ps.executeQuery();
            if (!rs.next()) {
                return false;
    ...
    public void store(String strNOM_RSO, String strDES_RSO, java.sql.Date dtDAT_RIL, String strNOM_RIL_RSO, String strCLF_RSO, long lPRB_EVE_LES, long lENT_DAN, long lFRQ_RIP_ATT_DAN, long lNUM_INC_INF, float lSTM_NUM_RSO, long lRFC_VLU_RSO_MES, long lCOD_FAT_RSO, java.util.ArrayList alAziende) {
        BMPConnection bmp = getConnection();
        try {
            PreparedStatement ps = bmp.prepareStatement("UPDATE ana_rso_can_tab SET nom_rso=?, des_rso=?, clf_rso=?, cod_fat_rso=? WHERE cod_rso=? AND (cod_azl=? OR cod_azl IN (" + toString(alAziende) + "))");
            ps.setString(1, strNOM_RSO);
            ps.setString(2, strDES_RSO);
            ps.setString(3, strCLF_RSO);
            ps.setLong(4, lCOD_FAT_RSO);
            ps.setLong(5, lCOD_RSO);
        ...
        try {
            {
                ...
                ps.executeUpdate();
                if (alAziende != null) {
                    PreparedStatement ps1 = bmp.prepareStatement("SELECT cod_azl FROM  ana_rso_can_tab WHERE cod_azl NOT IN (?) AND cod_rso=? AND cod_azl IN (" + toString(alAziende) + ")");
    ...
    public void removeCorso(long lCorso, java.util.ArrayList alAziende) {
        ...
        try {
            ...
            if (alAziende != null) {
                PreparedStatement ps4 = bmp.prepareStatement("SELECT cod_azl FROM  ana_rso_can_tab WHERE cod_azl NOT IN (?) AND cod_rso=? AND cod_azl IN (" + toString(alAziende) + ")");
    ...
    public void addNormativaSentenza(long lNormativa) {
        ...
        try {
            ...
            ps.executeUpdate();
            PreparedStatement ps1 = bmp.prepareStatement("SELECT cod_azl FROM  ana_rso_can_tab WHERE cod_azl NOT IN (?) AND cod_rso=? ");
            ps1.setLong(1, lCOD_AZL);
            ps1.setLong(2, lCOD_RSO);
            ResultSet rs = ps1.executeQuery();
            while (rs.next()) {
    ...
    public void removeDpi(long lDpi, java.util.ArrayList alAziende) {
        ...
        try {
            ...
            if (alAziende != null) {
                PreparedStatement ps4 = bmp.prepareStatement("SELECT cod_azl FROM  ana_rso_can_tab WHERE cod_azl NOT IN (?) AND cod_rso=? ");
                ps4.setLong(1, lCOD_AZL);
                ps4.setLong(2, lCOD_RSO);
                ResultSet rs = ps4.executeQuery();
                while (rs.next()) {
    ...
    public Collection ejbGetRischio_Nome_Fattore_View(long lAzienda) {
        ...
        try {
            PreparedStatement ps = bmp.prepareStatement(
                    "SELECT ana_rso_can_tab.cod_rso, ana_rso_can_tab.nom_rso, ana_fat_rso_tab.nom_fat_rso" + " FROM ana_rso_can_tab, ana_fat_rso_tab " + " WHERE ana_rso_can_tab.cod_fat_rso=ana_fat_rso_tab.cod_fat_rso AND " + " ana_rso_can_tab.cod_azl=? ORDER BY ana_fat_rso_tab.nom_fat_rso, ana_rso_can_tab.nom_rso");
            ps.setLong(1, lAzienda);
            ResultSet rs = ps.executeQuery();
...
/*
    ...
    public Collection ejbGetMisurePpView(long lCOD_RSO, long lCOD_AZL) {
        ...
        try {
            PreparedStatement ps = bmp.prepareStatement(
                    "SELECT " + " ana_mis_pet_tab.cod_mis_pet," + " ana_mis_pet_tab.nom_mis_pet," + " ana_mis_pet_tab.des_mis_pet" + " FROM ana_mis_pet_tab, rso_mis_pet_tab" + " WHERE " + " ana_mis_pet_tab.cod_mis_pet=rso_mis_pet_tab.cod_mis_pet" + " AND rso_mis_pet_tab.cod_rso=? and rso_mis_pet_tab.cod_azl=?");
            ps.setLong(1, lCOD_RSO);
            ps.setLong(2, lCOD_AZL);
            ResultSet rs = ps.executeQuery();
            java.util.ArrayList ar = new java.util.ArrayList();
...
*/
    public Collection getArt_Legge_View(){
        ...
        try{
            PreparedStatement ps=bmp.prepareStatement(
                "SELECT ANA_ARL_TAB.COD_ARL,ANA_ARL_TAB.NOM_ARL,ANA_ARL_TAB.DES_ARL FROM ANA_ARL_TAB, RSO_ARL_TAB WHERE RSO_ARL_TAB.COD_ARL=ana_arl_tab.cod_arl AND RSO_ARL_TAB.cod_rso=? AND RSO_ARL_TAB.cod_azl=? ORDER BY COD_ARL");
            ps.setLong(1, lCOD_RSO);
            ps.setLong(2, lCOD_AZL);
            ResultSet rs=ps.executeQuery();
            java.util.ArrayList al=new java.util.ArrayList();
...
//===========================================================
    public boolean ejbCaricaDbRischi(long P_COD_AZL, long P_COD_RSO) {
        ...
        try {
            ...
            long flag_esistenza;
            ps = bmp.prepareStatement("select count(*) from ana_rso_can_tab where cod_azl=? and cod_rso_rpo = ?");
            ...
            if (flag_esistenza == 0) {
                ps = bmp.prepareStatement(
                        "INSERT INTO ana_rso_can_tab(cod_azl, cod_rso, nom_rso, des_rso, dat_ril, " +
...
//----associate al protocollo sanitario
    ...
    public boolean ejbCaricaRpRischi(long P_COD_AZL, String P_NOM_RSO) {
        ...
        try {
            ...
            if (flag_esistenza == 0) {
                PreparedStatement ps_cod_rso = bmp.prepareStatement("SELECT cod_rso FROM ana_rso_can_tab " +
                ...
                ps = bmp.prepareStatement(
                        ...
                        "prb_eve_les, ent_dan, stm_num_rso, rfc_vlu_rso_mes, cod_fat_rso " +
                        "FROM ana_rso_can_tab WHERE nom_rso =? AND cod_azl=?");
                ps.setLong(1, V_COD_RSO);
                ps.setString(2, P_NOM_RSO);
                ps.setLong(3, P_COD_AZL);
                ps.executeUpdate();
                ps.close();
                ps = bmp.prepareStatement("update ana_rso_can_tab set cod_rso_rpo=? where nom_rso = ? and cod_azl = ?");
                ps.setLong(1, V_COD_RSO);
                ps.setString(2, P_NOM_RSO);
                ps.setLong(3, P_COD_AZL);
                ps.executeUpdate();
