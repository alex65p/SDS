...
public class RischioChimicoBean extends BMPEntityBean implements IRischioChimicoBean {
    ...
    private static RischioChimicoBean ys = null;

    private RischioChimicoBean() {
    }

    public static RischioChimicoBean getInstance() {
        if (ys == null) {
            ys = new RischioChimicoBean();
        }
        return ys;
    }
        ...
        try {
            ...
            if (this.COD_ALG == 1) // Rischio diretto per la sostanza
            {
                ...
                while (rs.next()) {
                    ...
                    if (sCbx.equalsIgnoreCase("QTA")) {
                        VAL_QTA = (new Double(rs.getDouble("val_pta"))).intValue();
                    }
                    if (sCbx.equalsIgnoreCase("TIP")) {
                        VAL_TIP = (new Double(rs.getDouble("val_pta"))).intValue();
                    }
                    if (sCbx.equalsIgnoreCase("CTR")) {
                        VAL_CTR = (new Double(rs.getDouble("val_pta"))).intValue();
                    }
                    if (sCbx.equalsIgnoreCase("TMP")) {
                        VAL_TMP = (new Double(rs.getDouble("val_pta"))).intValue();
                    }
                    if (sCbx.equalsIgnoreCase("DIS")) {
                        VAL_DIS = rs.getDouble("val_pta");
                    }
                }
                rs.close();
                ps.close();
            ...
            if (!this.TIP_RSO.equalsIgnoreCase("I")) {
                ...
                while (rs.next()) {
                    String sCbx = rs.getString("cod_cbx");
                    if (sCbx.equalsIgnoreCase("CCP")) {
                        VAL_CCP = (new Double(rs.getDouble("val_pta"))).intValue();
                    }
                    if (sCbx.equalsIgnoreCase("TIP")) {
                        VAL_TIP = (new Double(rs.getDouble("val_pta"))).intValue();
                    }
