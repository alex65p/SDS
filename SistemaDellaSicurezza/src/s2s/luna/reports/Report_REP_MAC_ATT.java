...
import com.apconsulting.luna.ejb.Macchina.AnagraficaDocumentiView;
import com.apconsulting.luna.ejb.Macchina.FornitoriMacView;
...
import com.apconsulting.luna.ejb.Macchina.Macchina_LuoghiFisiciView;
import com.apconsulting.luna.ejb.Macchina.NormativeSentenzeViewEx;
import com.apconsulting.luna.ejb.Macchina.ReportRischioMacchinaView;
import com.apconsulting.luna.ejb.Macchina.UtilizzatoriView;
import com.apconsulting.luna.ejb.TipologieMacchine.ITipologieMacchine;
import com.apconsulting.luna.ejb.TipologieMacchine.ITipologieMacchineHome;
 ...
 * @author Dario
 */
public class Report_REP_MAC_ATT extends Report {
    ...
    public void doReport() throws DocumentException, IOException, BadElementException, Exception { //----------------------------------------------------------------------
        ...
        initDocument("the doc", null, 
                ApplicationConfigurator.LanguageManager.getString("Scheda").toUpperCase() + "\n" 
                ...
                bean_az.getRAG_SCL_AZL(), bean.getDES_MAC());
        ...
        writeIndent();
        {
            ...
            tbl.addHeaderCellB(ApplicationConfigurator.LanguageManager.getString(
                    ApplicationConfigurator.isModuleEnabled(MODULES.MOD_FORM_GSE)
                        ?"Scheda.macchina.attrezzatura.impianto"
                        :"Scheda.macchina/attrezzatura"
                    ).toUpperCase());
            tbl.addTitleCell(bean.getDES_MAC(),"MACC_ATT");
            m_document.add(tbl);
        }
...
if (ApplicationConfigurator.isModuleEnabled(MODULES.ATT_LAV_MAC)){
        {//Attività lavorative associate
            CenterMiddleTable tbl = new CenterMiddleTable(1);
            tbl.toLeft();
            int width[] = {100};
            tbl.setWidths(width);

            tbl.addHeaderCellB(ApplicationConfigurator.LanguageManager.getString("Utilizzatori"));
            tbl.toCenter();

            Iterator it = bean.getAttivitaLavorativeMacchineView().iterator();
            while (it.hasNext()) {
                UtilizzatoriView w = (UtilizzatoriView) it.next();
                tbl.toLeft();
                tbl.addCell(Formatter.format(w.NOM_MAN));             
           }
            m_document.add(tbl);
        }}
