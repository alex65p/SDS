...
<%
    /*
     ...
     <description>ydalenie dannih iz TPL_DPI_TAB</description>
     </comment>		
     </comments> 
     </version>
     </versions>
...
<%
    ...
    long ID;
    long lCOD_NOR_SEN = 0, lCOD_DOC = 0, lCOD_LOT_DPI = 0;

    String LOCAL_MODE = request.getParameter("LOCAL_MODE");
    if (LOCAL_MODE != null) {
        ID = c.checkLong("Tipologia DPI", request.getParameter("ID_PARENT"), true);

        if (LOCAL_MODE.equals("ns")) {
            lCOD_NOR_SEN = c.checkLong("Normative Referemento", request.getParameter("ID"), true);
        }
    ...
    try {
        if (LOCAL_MODE != null) {
            tipologiaDPI = home.findByPrimaryKey(new Long(ID));

            if (lCOD_NOR_SEN != 0) {
                tipologiaDPI.removeCOD_NOR_SEN(lCOD_NOR_SEN);
            } else if (lCOD_DOC != 0) {
                tipologiaDPI.removeCOD_DOC(lCOD_DOC);
