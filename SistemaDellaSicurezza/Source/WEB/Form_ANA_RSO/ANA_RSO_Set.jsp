...
<%
    ...
    String strDES_RSO = c.checkString(ApplicationConfigurator.LanguageManager.getString("Descrizione.rischio"), request.getParameter("DES_RSO"), true);
    String strNOM_RIL_RSO = null;
    java.sql.Date dtDAT_RIL = null;
    long lRFC_VLU_RSO_MES = 0;
    if (!ifMsr) {
    ...
    dtDAT_RIL =c.checkDate(ApplicationConfigurator.LanguageManager.getString("Data.rilevazione"), request.getParameter("DAT_RIL"), true);
    lRFC_VLU_RSO_MES = c.checkLong(ApplicationConfigurator.LanguageManager.getString("Rifacimento.valutazione.(in.mesi)"), request.getParameter("RFC_VLU_RSO_MES"), true);
    }
    
    String strCLF_RSO = request.getParameter("CLF_RSO") + request.getParameter("CLF_RSO_TEXT");

    strCLF_RSO = c.checkString("Classificazione Rischio", strCLF_RSO, true);
    ...
    long lPRB_EVE_LES = c.checkLong(ApplicationConfigurator.LanguageManager.getString("Probabilità.evento.lesivo"), request.getParameter("PRB_EVE_LES"), true);
    long lENT_DAN = c.checkLong(ApplicationConfigurator.LanguageManager.getString("Entità.danno"), request.getParameter("ENT_DAN"), true);
