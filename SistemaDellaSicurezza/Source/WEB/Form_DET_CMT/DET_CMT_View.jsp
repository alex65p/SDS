...
<%-- 
    Document   : DET_CMT_View
    Created on : 06-mag-2008, 12.02.26
...
<div id="divContent">
    <%
        ...
        long lCOD_DPD = c.checkLong(ApplicationConfigurator.LanguageManager.getString("Nominativo"), request.getParameter("COD_DPD"), false);
        String strCOM_RES_TEL = c.checkStringEx(ApplicationConfigurator.LanguageManager.getString("Telefono"), request.getParameter("COM_RES_TEL"), false);

        Collection col = home.findEx_DettComm(
                lCOD_AZL,
                strPRO_CON,
                strDES_CON,
                lCOD_DPD,
    ...
    <table class="VIEW_TABLE" border="0">
        <thead>
            <tr class="VIEW_HEADER_TR">
                ...
                <th><%=ApplicationConfigurator.LanguageManager.getString("Qualifica")%></th>
            </tr>
        </thead>
        <tbody>
            <%
                int iCount = 0;
                while (it.hasNext()) {
                    DettComm_View view = (DettComm_View) it.next();
            %>
            <tr class="VIEW_TR" valign="top" INDEX="<%=view.COD_SRV%>" onclick="g_Handler.OnRowClick(this)" ondblclick="g_Handler.OnRowDblClick(this)">
                ...
                <%  
                        out.println("<td>" + ++iCount + "</td>"
                                ...
                                + "<td>" + Formatter.format(view.COG_DPD) + "  "
                                + Formatter.format(view.NOM_DPD) + "</td>"
                                + "<td>" + Formatter.format(view.NOM_FUZ_AZL) + "</td>"
                                + "</tr>");
                 }
                %>
        </tbody>
        <tfoot <%=!ApplicationConfigurator.isModuleEnabled(MODULES.ENHANCED_GRID) ? "style='display: none;'" : ""%>>
            <tr>
                ...
                <th><input type="text" name="cerca_Qualifica" value="Cerca <%=ApplicationConfigurator.LanguageManager.getString("Qualifica")%>" class="search_init" /></th>
            </tr>
        </tfoot>
    </table>
</div>
<script>
    ...
    function OnViewLoad() {
        parent.g_Handler.setCaptionHTML(getCompleteMenuPath(SubMenuContrattiServizi, 1));

        parent.g_Handler.New.URL = "/luna/WEB/Form_DET_CMT/DET_CMT_Form.jsp";
        parent.g_Handler.New.Width = 50;
        parent.g_Handler.New.Height = 41;

        parent.g_Handler.Open = parent.g_Handler.New;

        parent.g_Handler.Delete.URL = "/luna/WEB/Form_DET_CMT/DET_CMT_Delete.jsp";
        parent.g_Handler.Delete.OnBefore = OnBeforeDelete;
        parent.g_Handler.Help.URL = "/luna/WEB/Form_DET_CMT/DET_CMT_Help.jsp";
       parent.g_Handler.dataTableOn="Y";
        parent.g_Handler.dataTableArrayWidth=  [
                        ...
                        {"sWidth": "10%", "aTargets": [6], "sType": "string"} ,
                        {"sWidth": "10%", "aTargets": [7], "sType": "string"}   ]   ;
