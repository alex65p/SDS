...
long lCOD_VST_MED=c.checkLong(ApplicationConfigurator.LanguageManager.getString("Nome"),request.getParameter("COD_VST_MED"),true);
...
String strTPL_ACR_EFT=c.checkString("",request.getParameter("TPL_ACR_EFT"),true);
String strNOT_VST_MED=c.checkString(ApplicationConfigurator.LanguageManager.getString("Note"),request.getParameter("NOT_VST_MED"),false);
...
if(request.getParameter("SBM_MODE")!=null)
{
	...
	try
		 {
	long oldCOD_VST_MED=c.checkLong("COD_VST_MED",request.getParameter("oldCOD_VST_MED"),false);
    java.sql.Date oldDAT_PIF_VST=c.checkDate("DAT_PIF_VST",request.getParameter("oldDAT_PIF_VST"),false);
    bean.editCOD_VST_MED(oldCOD_VST_MED,lCOD_VST_MED,dtDAT_PIF_VST,dtDAT_EFT_VST,strTPL_ACR_VLU_RSO,strTPL_ACR_VLU_MED,strTPL_ACR_EFT,strNOT_VST_MED,oldDAT_PIF_VST);
			}																																																																						
	catch (Exception ex)
				{
	...
	try
		 {
		bean.addCOD_VST_MED(lCOD_VST_MED,dtDAT_PIF_VST,dtDAT_EFT_VST,strTPL_ACR_VLU_RSO,strTPL_ACR_VLU_MED,strTPL_ACR_EFT,strNOT_VST_MED);
		}
	catch (Exception ex)
				{
				out.print("<script>err=true;</script>");
		...
		if (isNew)
			 {
				...
				parent.ToolBar.OnNew('ID=<%=lCOD_VST_MED%>'); 
			}else{
