...
<%-- 
    Document   : SUB_APP_PER_COI_Delete
    Created on : 22-mag-2008, 9.28.55
...
<%
    ISubAppPersonaleCoinvolto SubAppPersonaleCoinvolto = null;
    String strCOD_PER_COI = request.getParameter("ID");
    if ( strCOD_PER_COI != null) {
        ISubAppPersonaleCoinvoltoHome home = (ISubAppPersonaleCoinvoltoHome) 
                PseudoContext.lookup("SubAppPersonaleCoinvoltoBean");
        try {
            long lCOD_SUB_APP = Long.parseLong(request.getParameter("ID_PARENT"));
            long lCOD_PER_COI = Long.parseLong(strCOD_PER_COI);
            home.remove(new SubAppPersonaleCoinvoltoPK(lCOD_SUB_APP,lCOD_PER_COI));
        } catch (Exception ex) {
