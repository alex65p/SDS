...
<%
	...
	try{
		...
		lCOD_PRO = Long.parseLong(strID);
	}catch(Exception ex){
		out.print(printErrAlert("divErr", "Error.showNotFound",ex));
		return;
	}
	if(strSubject.equals("PROCAN")){
		try{
			
					
						IAnagrCantieriHome ope_home=(IAnagrCantieriHome)PseudoContext.lookup("AnagrCantieriBean");
						IAnagrCantieri can_bean = ope_home.findByPrimaryKey(id_attachment);
