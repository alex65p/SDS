...
<file>
  <versions>	
    <version number="1.0" date="27/02/2004" author="Alexey Kolesnik">
      <comments>
			   <comment date="27/02/2004" author="Alexey Kolesnik">
				Anagrafica Protocolle Sanitare Attach
			   </comment>		
      </comments> 
    </version>
  </versions>
...
<%
	...
	try{
		...
		bean = home.findByPrimaryKey(new ProtocoleSanitarePK( lCOD_AZL, Long.parseLong(strID)));
	...
	try{
		if ("DOCUMENTI".equals(strSubject)){
			bean.addDocumento(id_attachment);
		}
		else if ("IDONEITA".equals(strSubject)){
			bean.addIdoneta(id_attachment);
		}
		else if ("MEDICHE".equals(strSubject)){
			bean.addMediche(id_attachment);
		}
		else{
			return;
		}
