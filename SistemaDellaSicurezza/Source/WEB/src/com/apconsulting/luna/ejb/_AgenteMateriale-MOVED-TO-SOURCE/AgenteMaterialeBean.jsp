    ...
    <version number="1.0" date="22/01/2004" author="Valentina Ruggieri">
	      <comments>
				  <comment date="22/01/2004" author="Valentina Ruggieri">
				   <description>AgenteMaterialeBean.jsp</description>
				 </comment>
  ...
  public void ejbRemove(){
      ...
      try
	  {
         PreparedStatement ps=bmp.prepareStatement("DELETE FROM ana_age_mat_tab  WHERE cod_age_mat=?");
          ps.setLong (1, COD_AGE_MAT);
          if(ps.executeUpdate()==0) throw new NoSuchEntityException("AgenteMateriale con ID= non è trovata");
      }
      catch(Exception ex)
	  {
          throw new EJBException(ex);
	...
	public Collection ejbfindEx(long COD_CAG, String NOM, long iOrderBy){
      ...
      try{
          PreparedStatement ps=bmp.prepareStatement(Sql);
					if(indexCag!=0){ps.setLong(indexCag, COD_CAG);}
					if(indexNom!=0){ps.setString(indexNom, NOM.toUpperCase());}
          ResultSet rs=ps.executeQuery();
          java.util.ArrayList al=new java.util.ArrayList();
          while(rs.next()){
