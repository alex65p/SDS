  ...
  public void ejbRemove(){
      ...
      try
      {
         PreparedStatement ps=bmp.prepareStatement("DELETE FROM ana_for_azl_tab  WHERE cod_for_azl=?");
          ps.setLong (1, COD_FOR_AZL);
          if(ps.executeUpdate()==0) throw new NoSuchEntityException("Fornitore con ID="+COD_FOR_AZL+" non è trovata");
      }
      catch(Exception ex)
      {
          throw new EJBException(ex);
    ...
    if(!isModified()) return;
      ...
      try{
         PreparedStatement ps=bmp.prepareStatement("UPDATE ana_for_azl_tab  SET rag_soc_for_azl=?, cag_for=?, idz_for=?, num_cic_for=?, cit_for=?, prv_for=?, cap_for=?, qlf_rsp_for=?, nom_rsp_for=?, idz_psa_elt_rsp=?, sit_int_for=?, cod_azl=?, cod_sta=? WHERE cod_for_azl=?");
          ...
          if(ps.executeUpdate()==0) throw new NoSuchEntityException("Fornitore with ID= not found");
      }
      catch(Exception ex){
          throw new EJBException(ex);
      }
...
//<comment description="setter/getters">
    ...
    public String getCAG_FOR(){
      ...
      if((CAG_FOR.equals(newCAG_FOR)) && (RAG_SOC_FOR_AZL.equals(newRAG_SOC_FOR_AZL))) return;
  ...
  public void setIDZ_FOR(String newIDZ_FOR){
      if(IDZ_FOR.equals(newIDZ_FOR)) return;
      IDZ_FOR = newIDZ_FOR;
      setModified();
    }
  ...
  public void setNUM_CIC_FOR(String newNUM_CIC_FOR){
      if(NUM_CIC_FOR != null)
                if(NUM_CIC_FOR.equals(newNUM_CIC_FOR)) return;
      NUM_CIC_FOR = newNUM_CIC_FOR;
      setModified();
    }
    public String getNUM_CIC_FOR(){
      if(NUM_CIC_FOR==null){return "";}
            return NUM_CIC_FOR;
    }
    //----5
  public void setCIT_FOR(String newCIT_FOR){
      if(CIT_FOR.equals(newCIT_FOR)) return;
      CIT_FOR = newCIT_FOR;
      setModified();
    }
  ...
  public void setPRV_FOR(String newPRV_FOR){
      if(PRV_FOR != null) if(PRV_FOR.equals(newPRV_FOR)) return;
      PRV_FOR = newPRV_FOR;
      setModified();
    }
    public String getPRV_FOR(){
     if(PRV_FOR==null){return "";}
      return PRV_FOR;
    }
    //----7
  public void setCAP_FOR(String newCAP_FOR){
      if(CAP_FOR != null) if(CAP_FOR.equals(newCAP_FOR)) return;
      CAP_FOR = newCAP_FOR;
      setModified();
    }
    public String getCAP_FOR(){
      if(CAP_FOR==null){return "";}
            return CAP_FOR;
    }
    //----8
  public void setQLF_RSP_FOR(String newQLF_RSP_FOR){
      if(QLF_RSP_FOR.equals(newQLF_RSP_FOR)) return;
  ...
  public void setNOM_RSP_FOR(String newNOM_RSP_FOR){
      if(NOM_RSP_FOR.equals(newNOM_RSP_FOR)) return;
  ...
  public void setIDZ_PSA_ELT_RSP(String newIDZ_PSA_ELT_RSP){
      if(IDZ_PSA_ELT_RSP != null) if(IDZ_PSA_ELT_RSP.equals(newIDZ_PSA_ELT_RSP)) return;
      IDZ_PSA_ELT_RSP = newIDZ_PSA_ELT_RSP;
      setModified();
    }
    public String getIDZ_PSA_ELT_RSP(){
      if(IDZ_PSA_ELT_RSP==null){return "";}
            return IDZ_PSA_ELT_RSP;
    }
    //----11
  public void setSIT_INT_FOR(String newSIT_INT_FOR){
      if(SIT_INT_FOR != null) if(SIT_INT_FOR.equals(newSIT_INT_FOR)) return;
      SIT_INT_FOR = newSIT_INT_FOR;
      setModified();
    }
    public String getSIT_INT_FOR(){
      if(SIT_INT_FOR==null){return "";}
            return SIT_INT_FOR;
    }
    //----------------12
...
// %%%UNLink%%% Table SOS_CHI_FOR_AZL_TAB
    public void removeCOD_SOS_CHI(long newCOD_SOS_CHI){
      ...
      try
      {
         ...
         if(ps.executeUpdate()==0) throw new NoSuchEntityException("Agente Chimice con ID="+newCOD_SOS_CHI+" non &egrave trovata");
      }
      catch(Exception ex)
      {
         throw new EJBException(ex);
...
// %%%UNLink%%% Table mac_for_azl_tab
    public void removeCOD_MAC(long newCOD_MAC){
      ...
      try
      {
         ...
         if(ps.executeUpdate()==0) throw new NoSuchEntityException("Macchina con ID="+newCOD_MAC+" non &egrave trovata");
      }
      catch(Exception ex)
      {
         throw new EJBException(ex);
	...
	public Collection ejbFindEx(  
		...
		BMPConnection bmp=getConnection();
			try{
				...
				if(strRAG_SOC_FOR_AZL!=null) ps.setString(i++, strRAG_SOC_FOR_AZL.toUpperCase());
				if(strCAG_FOR!=null) ps.setString(i++, strCAG_FOR.toUpperCase());
				if(strIDZ_FOR!=null) ps.setString(i++, strIDZ_FOR.toUpperCase());
				if(strNUM_CIC_FOR!=null) ps.setString(i++, strNUM_CIC_FOR.toUpperCase());
				if(strCIT_FOR!=null) ps.setString(i++, strCIT_FOR.toUpperCase());
				if(strPRV_FOR!=null) ps.setString(i++, strPRV_FOR.toUpperCase());
				if(strCAP_FOR!=null) ps.setString(i++, strCAP_FOR.toUpperCase());
				if(strQLF_RSP_FOR!=null) ps.setString(i++, strQLF_RSP_FOR.toUpperCase());
				if(strNOM_RSP_FOR!=null) ps.setString(i++, strNOM_RSP_FOR.toUpperCase());
				if(strIDZ_PSA_ELT_RSP!=null) ps.setString(i++, strIDZ_PSA_ELT_RSP.toUpperCase());
				if(strSIT_INT_FOR!=null) ps.setString(i++, strSIT_INT_FOR.toUpperCase());
				//----------------------------------------------------------------------
				ResultSet rs=ps.executeQuery();
...
PseudoContext.bind("FornitoreBean", new FornitoreBean());
////////////////////////////////////////// </BINDING> /////////////////////////////////////////////////////////////////
%>
