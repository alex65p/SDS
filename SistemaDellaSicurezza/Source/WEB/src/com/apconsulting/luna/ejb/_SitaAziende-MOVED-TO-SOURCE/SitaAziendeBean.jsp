...
<description>SitaAziendeBean.jsp</description>
</comment>
...
//System.err.println("SitaAziendeBean constructor<br>");
    }
...
//----------------------------------------------------------
    ...
    public void ejbRemove(){
        ...
        try {
            ...
            ps.setLong(1, COD_SIT_AZL);
            if(ps.executeUpdate()==0) throw new NoSuchEntityException("SitaAziende con ID= non è trovata");
        } catch(Exception ex) {
            throw new EJBException(ex);
        } finally{bmp.close();
        }
...
//------------------------
    public void setCOD_AZL_NOM_SIT_AZL(long newCOD_AZL, String newNOM_SIT_AZL){
        if( (COD_AZL==newCOD_AZL )&(NOM_SIT_AZL.equals(newNOM_SIT_AZL)) ) return;
        COD_AZL = newCOD_AZL;
...
//5
    public void setNUM_CIC_SIT_AZL(String newNUM_CIC_SIT_AZL){
        if(NUM_CIC_SIT_AZL!=null){
            if( NUM_CIC_SIT_AZL.equals(newNUM_CIC_SIT_AZL) ) return;
        }
    ...
    public String getNUM_CIC_SIT_AZL(){
        if(NUM_CIC_SIT_AZL==null) {
            return "";}else{return NUM_CIC_SIT_AZL;}
    }
    
//6
    public void setPRV_SIT_AZL(String newPRV_SIT_AZL){
        if(PRV_SIT_AZL!=null){
            if( PRV_SIT_AZL.equals(newPRV_SIT_AZL) ) return;
        }
    ...
    public String getPRV_SIT_AZL(){
        if(PRV_SIT_AZL==null) {
            return "";}else{return PRV_SIT_AZL;}
    }
//7
    public void setCAP_SIT_AZL(String newCAP_SIT_AZL){
        if(CAP_SIT_AZL!=null){
            if( CAP_SIT_AZL.equals(newCAP_SIT_AZL) ) return;
        }
    ...
    public String getCAP_SIT_AZL(){
        if(CAP_SIT_AZL==null) {
            return "";}else{return CAP_SIT_AZL;}
    }
...
//<comment description="extended setters/getters">
    public Collection ejbGetSiteAziendaleByAZLID_View(long AZL_ID){
        BMPConnection bmp=getConnection();
        try{
            PreparedStatement ps=bmp.prepareStatement("SELECT cod_sit_azl,nom_sit_azl,idz_sit_azl,num_cic_sit_azl,cit_sit_azl,prv_sit_azl,cap_sit_azl FROM ana_sit_azl_tab WHERE cod_azl=? ORDER BY nom_sit_azl");
            ps.setLong(1, AZL_ID);
            ResultSet rs=ps.executeQuery();
            java.util.ArrayList al=new java.util.ArrayList();
            while(rs.next()){
    ...
    public Collection ejbGetNonEmptySiteAziendaleByAZLID_View(long AZL_ID){
        ...
        try{
            PreparedStatement ps=bmp.prepareStatement(
                    " SELECT a.COD_SIT_AZL, a.NOM_SIT_AZL "+
                    " FROM   ANA_SIT_AZL_TAB a "+
                    " WHERE A.COD_AZL = ? "+
                    " AND EXISTS (SELECT 'x' FROM   ana_luo_fsc_tab b WHERE  b.cod_sit_azl = a.cod_sit_azl AND B.COD_AZL = ?) "+
                    " ORDER BY 2 "
                    );
            ps.setLong(1, AZL_ID);
            ps.setLong(2, AZL_ID);
...
//</comment>
    ...
    public Collection ejbFindEx(long	AZL_ID,
        ...
        String strSql = "SELECT cod_sit_azl, UPPER(nom_sit_azl) AS nom_sit_azl, idz_sit_azl, num_cic_sit_azl, cit_sit_azl, prv_sit_azl, cap_sit_azl  "+
                "FROM ana_sit_azl_tab b WHERE b.cod_azl=?";
...
PseudoContext.bind("SitaAziendeBean", new SitaAziendeBean());
////////////////////////////////////////// </BINDING> /////////////////////////////////////////////////////////////////
%>
