...
////////////////////// CONSTRUCTOR///////////////////
 private CategoriePresideBean()
  {
	//System.err.println("CategoriePresideBean constructor<br>");
  }
  ...
  public void ejbRemove(){
      ...
      try
	  {
         PreparedStatement ps=bmp.prepareStatement("DELETE FROM cag_psd_acd_tab  WHERE cod_cag_psd_acd=?");
          ps.setLong (1, COD_CAG_PSD_ACD);
          if(ps.executeUpdate()==0) throw new NoSuchEntityException("CategoriePreside con ID="+COD_CAG_PSD_ACD+" non è trovata");
      }
      catch(Exception ex)
	  {
          throw new EJBException(ex);
    ...
    if(!isModified()) return;
      ...
      try{
         PreparedStatement ps=bmp.prepareStatement("UPDATE cag_psd_acd_tab  SET nom_cag_psd_acd=?, des_cag_psd_acd=?, per_mes_sst=?, per_mes_mnt=?  WHERE cod_cag_psd_acd=?");
          ...
          if(ps.executeUpdate()==0) throw new NoSuchEntityException("This CategoriePreside not found");
      }
      catch(Exception ex){
          throw new EJBException(ex);
      }
...
//<comment description="setter/getters">  
	//----1
      ...
      if(NOM_CAG_PSD_ACD.equals(newNOM_CAG_PSD_ACD)) return;
  ...
  public void setDES_CAG_PSD_ACD(String newDES_CAG_PSD_ACD){
      if(DES_CAG_PSD_ACD != null) 
				if(DES_CAG_PSD_ACD.equals(newDES_CAG_PSD_ACD)) return;
      DES_CAG_PSD_ACD = newDES_CAG_PSD_ACD;
      setModified();
    }
    public String getDES_CAG_PSD_ACD(){
      if(DES_CAG_PSD_ACD==null){return "";}
...
//<report>
 ...
 public Collection ejbGetCategoriePresidiView(long lCOD_CAG_PSD_ACD){
	BMPConnection bmp=getConnection();
      try{
          PreparedStatement ps=bmp.prepareStatement("select a.cod_cag_psd_acd , a.nom_cag_psd_acd , a.des_cag_psd_acd  , a.per_mes_sst   ,   a.per_mes_mnt  ,           c.nom_luo_fsc, b.cod_psd_acd from   cag_psd_acd_tab a, ana_psd_acd_tab b, ana_luo_fsc_tab c where a.cod_cag_psd_acd in (?) and       a.cod_cag_psd_acd=b.cod_cag_psd_acd and b.cod_luo_fsc=c.cod_luo_fsc ");
  ...
  //
    public Collection ejbGetCAG_Lov(long lCOD_AZL, String strNOM_CAG_PSD_ACD){
     ...
     try{
       ...
       "SELECT DISTINCT a.cod_cag_psd_acd, a.nom_cag_psd_acd, a.per_mes_sst, a.per_mes_mnt "
                                        +" FROM cag_psd_acd_tab a, ana_psd_acd_tab b, ana_luo_fsc_tab c "
                                        +" WHERE a.cod_cag_psd_acd = b.cod_cag_psd_acd AND b.cod_luo_fsc=c.cod_luo_fsc AND c.cod_azl=? "
                                        +" AND a.nom_cag_psd_acd LIKE ?||'%' ORDER BY a.nom_cag_psd_acd ");
	...
	public Collection ejbFindEx( String strNOM_CAG_PSD_ACD, 
		...
		BMPConnection bmp=getConnection();
			try{
				...
				if(strNOM_CAG_PSD_ACD!=null) ps.setString(i++, strNOM_CAG_PSD_ACD.toUpperCase());
				if(strDES_CAG_PSD_ACD!=null) ps.setString(i++, strDES_CAG_PSD_ACD.toUpperCase());
				if(lPER_MES_SST!=null) ps.setLong(i++, lPER_MES_SST.longValue());
				if(lPER_MES_MNT!=null) ps.setLong(i++, lPER_MES_MNT.longValue());
...
PseudoContext.bind("CategoriePresideBean", new CategoriePresideBean());
////////////////////////////////////////// </BINDING> /////////////////////////////////////////////////////////////////
%>
