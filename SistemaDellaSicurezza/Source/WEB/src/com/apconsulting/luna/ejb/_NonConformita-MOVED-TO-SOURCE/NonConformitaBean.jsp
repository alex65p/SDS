...
//public class NonConformitaBean extends BMPEntityBean implements INonConformitaRemote
...
////////////////////// CONSTRUCTOR///////////////////
 private NonConformitaBean()
  {
	//System.err.println("NonConformitaBean constructor<br>");
  }
  ...
  public void ejbRemove(){
      ...
      try
	  {
         PreparedStatement ps=bmp.prepareStatement("DELETE FROM ana_non_cfo_tab  WHERE cod_non_cfo=?");
          ps.setLong (1, COD_NON_CFO);
          if(ps.executeUpdate()==0) throw new NoSuchEntityException("NonConformita con ID= non è trovata");
      }
      catch(Exception ex)
	  {
          throw new EJBException(ex);
      ...
      try{
         PreparedStatement ps=bmp.prepareStatement("UPDATE ana_non_cfo_tab  SET des_non_cfo=?, dat_ril_non_cfo=?, nom_ril_non_cfo=?, oss_non_cfo=?, cod_inr_adt=?, cod_azl=? WHERE cod_non_cfo=?");
          ...
          ps.setDate  (2,DAT_RIL_NON_CFO);
					...
					if(COD_INR_ADT==0){ps.setNull (5,java.sql.Types.BIGINT);}else{ps.setLong (5,COD_INR_ADT);}
...
//<comment description="setter/getters">
    //1
      ...
      if( DES_NON_CFO.equals(newDES_NON_CFO) ) return;
    ...
    public String getDES_NON_CFO(){
      ...
      if( DAT_RIL_NON_CFO.equals(newDAT_RIL_NON_CFO) ) return;
    ...
    public java.sql.Date getDAT_RIL_NON_CFO(){
      ...
      if( NOM_RIL_NON_CFO.equals(newNOM_RIL_NON_CFO) ) return;
	...
	//6
      ...
      if(OSS_NON_CFO!=null){
	      if( OSS_NON_CFO.equals(newOSS_NON_CFO) ) return;
      }
	  OSS_NON_CFO = newOSS_NON_CFO;
	  setModified();
    }
     public String getOSS_NON_CFO(){
		  if (OSS_NON_CFO==null) return "";
      return OSS_NON_CFO;
    }
...
//          PreparedStatement ps=bmp.prepareStatement("select a.cod_non_cfo, a.des_non_cfo,a.cod_mis_pet, dat_ril_non_cfo,a.nom_ril_non_cfo,a.cod_inr_adt, a.oss_non_cfo, b.des_inr_adt from ana_non_cfo_tab a left outer join ana_inr_adt_tab b on a.cod_inr_adt = b.cod_inr_adt where   a.cod_inr_adt = ?  ");
					PreparedStatement ps=bmp.prepareStatement
                            (SQLContainer.getEjbGetNonConformitaByInterventoAudit());
...
//          PreparedStatement ps=bmp.prepareStatement("select a.cod_non_cfo, a.des_non_cfo,a.cod_mis_pet, dat_ril_non_cfo,a.nom_ril_non_cfo,a.cod_inr_adt, a.oss_non_cfo, b.des_inr_adt from ana_non_cfo_tab a left outer join ana_inr_adt_tab b on a.cod_inr_adt = b.cod_inr_adt where   a.cod_azl = ?  ");
					PreparedStatement ps=bmp.prepareStatement
                            (SQLContainer.getEjbGetNonConformita());
	...
	public Collection ejbFindEx( 
		...
		BMPConnection bmp=getConnection();
			try{
				...
				if(strDES_NON_CFO!=null) ps.setString(i++, strDES_NON_CFO.toUpperCase());
				if(strOSS_NON_CFO!=null) ps.setString(i++, strOSS_NON_CFO.toUpperCase());
				if(dtDAT_RIL_NON_CFO!=null) ps.setDate(i++, dtDAT_RIL_NON_CFO);
				if(strNOM_RIL_NON_CFO!=null) ps.setString(i++, strNOM_RIL_NON_CFO.toUpperCase());
				if(lCOD_INR_ADT!=null) ps.setLong(i++, lCOD_INR_ADT.longValue());
...
///////////ATTENTION!!////////////////////////////////////////
}//<comment description="end of implementation  NonConformitaBean"/>
%>
<%
////////////////////////////////////////// <BINDING> ////////////
PseudoContext.bind("NonConformitaBean", new NonConformitaBean());
////////////////////////////////////////// </BINDING> /////////// 
%>
