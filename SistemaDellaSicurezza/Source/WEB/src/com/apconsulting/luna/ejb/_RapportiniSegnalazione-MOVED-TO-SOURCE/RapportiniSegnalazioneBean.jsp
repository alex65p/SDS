...
public class RapportiniSegnalazioneBean extends BMPEntityBean implements IRapportiniSegnalazione, IRapportiniSegnalazioneHome
{
 ...
 //<IRapportiniSegnalazioneHome-implementation>
      ...
      public RapportiniSegnalazioneBean(){}
  ...
  //  <VIEWS>
      ...
      public Collection getMax_Numero_View()
      {
		 ...
		 public Collection findEx( 
			...
			int iOrderParameter)
	 {
	      	return (new  RapportiniSegnalazioneBean()).ejbFindEx(lCOD_DPD,strDES_SGZ,dtDAT_SGZ,lNUM_SGZ,lVER_SGZ,strTIT_SGZ,strSTA_SGZ,strURG_SGZ,strDES_ATI_SGZ,strSTA_FIE_SGZ,dtDAT_FIE,strNOM_RIL_SGZ, lCOD_AZL, iOrderParameter);
					}	
  //</VIEWS>
 ...
 //
  ...
  public void ejbLoad(){
      BMPConnection bmp=getConnection();
      try{
         PreparedStatement ps=bmp.prepareStatement("SELECT cod_sgz,des_sgz,dat_sgz,num_sgz,ver_sgz,tit_sgz,sta_sgz,urg_sgz,des_ati_sgz,sta_fie_sgz,dat_fie,nom_ril_sgz,cod_dpd,cod_azl FROM ana_sgz_tab WHERE cod_sgz=?");
           ps.setLong(1, lCOD_SGZ);
           ResultSet rs=ps.executeQuery();
           if(rs.next()){
    			...
    			lCOD_DPD=rs.getLong(13);
    			lCOD_AZL=rs.getLong(14);
           }
           else{
  ...
  public void ejbRemove(){
      ...
      try{
         ...
         ps.setLong(1, lCOD_SGZ);
         if(ps.executeUpdate()==0) throw new NoSuchEntityException("RapportiniSegnalazione con ID="+lCOD_SGZ+" non &egrave trovata");
      }
      catch(Exception ex){
      	ex.printStackTrace(System.err);
          throw new EJBException(ex);
  ...
  public void ejbStore()
  {
    if(!isModified()) return;
      BMPConnection bmp=getConnection();
      try{
         PreparedStatement ps=bmp.prepareStatement("UPDATE ana_sgz_tab SET cod_sgz=?, des_sgz=?, dat_sgz=?, num_sgz=?, ver_sgz=?, tit_sgz=?, sta_sgz=?, urg_sgz=?, des_ati_sgz=?, sta_fie_sgz=?, dat_fie=?, nom_ril_sgz=?, cod_dpd=?, cod_azl=? WHERE cod_sgz=?");
			ps.setLong(1, lCOD_SGZ);
			ps.setString(2, strDES_SGZ);
			ps.setDate(3, dtDAT_SGZ);
			ps.setLong(4, lNUM_SGZ);
			...
			ps.setLong(13, lCOD_DPD);
			ps.setLong(14, lCOD_AZL);
			ps.setLong(15, lCOD_SGZ);
  ...
  //<setter-getters>
	...
	public String getDES_SGZ(){
		if(strDES_SGZ==null){return "";}
		return strDES_SGZ;
	}
	public void setDES_SGZ(String strDES_SGZ){
		if(  (this.strDES_SGZ!=null) && (this.strDES_SGZ.equals(strDES_SGZ))  ) return;
		this.strDES_SGZ=strDES_SGZ;
		setModified();
	}
	...
	public void setDAT_SGZ(java.sql.Date dtDAT_SGZ){
		if(this.dtDAT_SGZ==dtDAT_SGZ) return;
		this.dtDAT_SGZ=dtDAT_SGZ;
		setModified();
	}
	...
	public void setNUM_SGZ(long lNUM_SGZ){
		if(this.lNUM_SGZ==lNUM_SGZ) return;
		this.lNUM_SGZ=lNUM_SGZ;
		setModified();
	}
	...
	public void setVER_SGZ(long lVER_SGZ){
		if(this.lVER_SGZ==lVER_SGZ) return;
		this.lVER_SGZ=lVER_SGZ;
		setModified();
	}

	public String getTIT_SGZ(){
		if(strTIT_SGZ==null){return "";}
		return strTIT_SGZ;
	}
	public void setTIT_SGZ(String strTIT_SGZ){
		if(  (this.strTIT_SGZ!=null) && (this.strTIT_SGZ.equals(strTIT_SGZ))  ) return;
		this.strTIT_SGZ=strTIT_SGZ;
		setModified();
	}

	public String getSTA_SGZ(){
		if(strSTA_SGZ==null){return "";}
		return strSTA_SGZ;
	}

	public void setSTA_SGZ(String strSTA_SGZ){
		if(  (this.strSTA_SGZ!=null) && (this.strSTA_SGZ.equals(strSTA_SGZ))  ) return;
		this.strSTA_SGZ=strSTA_SGZ;
		setModified();
	}

	public String getURG_SGZ(){
		if(strURG_SGZ==null){return "";}
		return strURG_SGZ;
	}

	public void setURG_SGZ(String strURG_SGZ){
		if(  (this.strURG_SGZ!=null) && (this.strURG_SGZ.equals(strURG_SGZ))  ) return;
		this.strURG_SGZ=strURG_SGZ;
		setModified();
	}

	public String getDES_ATI_SGZ(){
		if(strDES_ATI_SGZ==null){return "";}
	...
	public String getSTA_FIE_SGZ(){
		if(strSTA_FIE_SGZ==null){return "";}
		return strSTA_FIE_SGZ;
	}
	public void setSTA_FIE_SGZ(String strSTA_FIE_SGZ){
		if(  (this.strSTA_FIE_SGZ!=null) && (this.strSTA_FIE_SGZ.equals(strSTA_FIE_SGZ))  ) return;
	...
	public String getNOM_RIL_SGZ(){
		if(strNOM_RIL_SGZ==null){return "";}
		return strNOM_RIL_SGZ;
	}
	public void setNOM_RIL_SGZ(String strNOM_RIL_SGZ){
		if(  (this.strNOM_RIL_SGZ!=null) && (this.strNOM_RIL_SGZ.equals(strNOM_RIL_SGZ))  ) return;
		this.strNOM_RIL_SGZ=strNOM_RIL_SGZ;
		setModified();
	}
	...
	public void setVER_SGZ__TIT_SGZ(long lVER_SGZ, String strTIT_SGZ){
	    if( ( this.lVER_SGZ == lVER_SGZ ) && this.strTIT_SGZ.equals(strTIT_SGZ) ) return;
		this.lVER_SGZ=lVER_SGZ;
		this.strTIT_SGZ=strTIT_SGZ;
		setModified();
    }
...
if( (dDAT_SGZ_DAL!=null) && (dDAT_SGZ_AL!=null) ){
  vWhere+=" AND a.dat_sgz BETWEEN ? AND ?";
	ifDAT_SGZ_DAL = index++; ifDAT_SGZ_AL = index++;
}
if( (dDAT_SGZ_DAL!=null) && (dDAT_SGZ_AL==null) ){
...
if( "G".equals(strSTA_INT) ){
  if( (dDAT_SCA_DAL!=null) && (dDAT_SCA_AL!=null) ){
    vWhere+=" AND b.dat_sca BETWEEN ? AND ?";
  	ifDAT_SCA_DAL = index++; ifDAT_SCA_AL = index++;
  }
  if( (dDAT_SCA_DAL!=null) && (dDAT_SCA_AL==null) ){
...
try{
  PreparedStatement ps=bmp.prepareStatement("SELECT a.cod_sgz,b.cod_ati_sgz,a.dat_sgz,b.dat_sca,c.cog_dpd,c.nom_dpd,d.rag_scl_azl,d.cod_azl FROM ana_sgz_tab a, ana_ati_sgz_tab b, view_ana_dpd_tab c, ana_azl_tab d WHERE a.cod_sgz = b.cod_sgz AND a.cod_dpd = c.cod_dpd AND a.cod_azl = d.cod_azl AND d.cod_azl = ? " + vWhere + vGroup);
	...
	if (ifDAT_SGZ_DAL!=0) ps.setDate(ifDAT_SGZ_DAL, dDAT_SGZ_DAL);
	if (ifDAT_SGZ_AL !=0) ps.setDate(ifDAT_SGZ_AL , dDAT_SGZ_AL);
	if (ifDAT_SCA_DAL!=0) ps.setDate(ifDAT_SCA_DAL, dDAT_SCA_DAL);
	if (ifDAT_SCA_AL !=0) ps.setDate(ifDAT_SCA_AL , dDAT_SCA_AL);
	if (ifCOD_DPD    !=0) ps.setLong(ifCOD_DPD, lCOD_DPD);
	if (ifTIT_SGZ    !=0) ps.setString(ifTIT_SGZ, strTIT_SGZ);
	if (ifNOM_RIL_SGZ!=0) ps.setString(ifNOM_RIL_SGZ, strNOM_RIL_SGZ);
  ResultSet rs=ps.executeQuery();
  java.util.ArrayList al=new java.util.ArrayList();
  while(rs.next()){
...
<%
///////////////////////////////////////////////////////////////////////////////////////////
PseudoContext.bind(RapportiniSegnalazioneBean.BEAN_NAME, new RapportiniSegnalazioneBean());
///////////////////////////////////////////////////////////////////////////////////////////
%>
