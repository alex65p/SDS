...
//<comment description="setter/getters">
	...
	public void setCOD_PSD_ACD(long newCOD_PSD_ACD){
      if( COD_PSD_ACD==newCOD_PSD_ACD ) return;
	...
	public void setTPL_INR_PSD(String newTPL_INR_PSD){
      if( TPL_INR_PSD.equals(newTPL_INR_PSD) ) return;
	...
	public void setATI_SVO(String newATI_SVO){
      if( ( ATI_SVO!=null)&&(ATI_SVO.equals(newATI_SVO))) return;
      ATI_SVO = newATI_SVO;
      setModified();
    }
...
//----------------------------------------------------------------
	 public Collection ejbGetSchedeInterventoPSD_SelectData_View()
	 {
       BMPConnection bmp=getConnection();
       try{
          PreparedStatement ps=bmp.prepareStatement("SELECT a.cod_psd_acd, a.ide_psd_acd, b.nom_cag_psd_acd, c.nom_luo_fsc FROM ana_psd_acd_tab a, cag_psd_acd_tab b, ana_luo_fsc_tab c WHERE  a.cod_cag_psd_acd=b.cod_cag_psd_acd AND a.cod_luo_fsc=c.cod_luo_fsc ");
    	  ResultSet rs=ps.executeQuery();
          java.util.ArrayList al=new java.util.ArrayList();
          while(rs.next()){
	...
	public Collection ejbGetSchedeInterventoPSD_Responsabile_View(long COD_PSD_ACD, String WHE_IN_AZL){
       BMPConnection bmp=getConnection();
       try{
		  String query = "SELECT a.cod_sch_inr_psd, a.nom_rsp_inr ";
		  query += "FROM  sch_inr_psd_tab a, ana_psd_acd_tab b, ana_luo_fsc_tab c ";
		  query += "WHERE a.cod_psd_acd = b.cod_psd_acd ";
		  if (COD_PSD_ACD != 0) query += "AND b.cod_psd_acd = ? ";
		  query += "AND b.cod_luo_fsc = c.cod_luo_fsc AND a.nom_rsp_inr is not null AND c.cod_azl IN ("+WHE_IN_AZL+")";
          PreparedStatement ps=bmp.prepareStatement(query);
		  if (COD_PSD_ACD != 0) ps.setLong(1, COD_PSD_ACD);
...
//   Create View dlya Presidi
    ...
    public void removeCOD_DOC(long newCOD_DOC){
      ...
      catch(Exception ex)
      {
       ...
       BMPConnection bmp=getConnection();
       try{
          PreparedStatement ps=bmp.prepareStatement("SELECT a.cod_psd_acd, a.ide_psd_acd, d.nom_cag_psd_acd, e.nom_luo_fsc, b.cod_sch_inr_psd, b.ati_svo, b.dat_inr, b.pbm_rsc, b.dat_pif_inr, b.nom_rsp_inr, b.tpl_inr_psd, b.esi_inr FROM ana_psd_acd_tab a, sch_inr_psd_tab b, cag_psd_acd_tab d, ana_luo_fsc_tab e WHERE a.cod_psd_acd = b.cod_psd_acd AND a.cod_cag_psd_acd = d.cod_cag_psd_acd AND a.cod_luo_fsc = e.cod_luo_fsc AND a.sta_psd_acd='A' ORDER BY a.ide_psd_acd ");
		  ResultSet rs=ps.executeQuery();
          java.util.ArrayList al=new java.util.ArrayList();
          while(rs.next())
		  {
	...
	public Collection ejbFindEx( 
		...
		String strSql="SELECT a.cod_psd_acd, a.ide_psd_acd, d.nom_cag_psd_acd, e.nom_luo_fsc, b.cod_sch_inr_psd, b.ati_svo, b.dat_inr, b.pbm_rsc, b.dat_pif_inr, b.nom_rsp_inr, b.tpl_inr_psd, b.esi_inr FROM ana_psd_acd_tab a, sch_inr_psd_tab b, cag_psd_acd_tab d, ana_luo_fsc_tab e WHERE a.cod_psd_acd = b.cod_psd_acd AND a.cod_cag_psd_acd = d.cod_cag_psd_acd AND a.cod_luo_fsc = e.cod_luo_fsc AND a.sta_psd_acd='A' ";
		...
		try{
				...
				if(strTPL_INR_PSD!=null) ps.setString(i++, strTPL_INR_PSD.toUpperCase());
				if(lCOD_PSD_ACD!=null) ps.setLong(i++, lCOD_PSD_ACD.longValue());
				if(strATI_SVO!=null) ps.setString(i++, strATI_SVO.toUpperCase());
				if(dtDAT_INR!=null) ps.setDate(i++, dtDAT_INR);
				if(dtDAT_PIF_INR!=null) ps.setDate(i++, dtDAT_PIF_INR);
				if(strNOM_RSP_INR!=null) ps.setString(i++, strNOM_RSP_INR.toUpperCase());
				ResultSet rs=ps.executeQuery();
...
<%
////////////////////////////////////////// <BINDING> ////////////////////////
PseudoContext.bind("SchedeInterventoPSDBean", new SchedeInterventoPSDBean());
////////////////////////////////////////// </BINDING> ///////////////////////
%>
