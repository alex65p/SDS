...
public class CartelleSanitarieBean extends BMPEntityBean implements ICartelleSanitarie, ICartelleSanitarieHome
//class CartelleSanitarieBean extends ICartelleSanitarieHome
{
...
////////////////////// CONSTRUCTOR///////////////////
 private CartelleSanitarieBean()
  {
	//System.err.println("CartelleSanitarieBean constructor<br>");
  }
  ...
  public void ejbRemove(){
	  ...
	  try
	  {
		 PreparedStatement ps=bmp.prepareStatement("DELETE FROM ana_ctl_san_tab  WHERE cod_ctl_san=?");
		  ps.setLong (1, COD_CTL_SAN);
		  if(ps.executeUpdate()==0) throw new NoSuchEntityException("Cartelle Sanitarie con ID= non è trovata");
	  }
	  catch(Exception ex)
	  {
		  throw new EJBException(ex);
  ...
  public void ejbStore()
  {
	if(!isModified()) return;
	  ...
	  try{
		 ...
		 if(ps.executeUpdate()==0) throw new NoSuchEntityException("CartelleSanitarie con ID= non è trovata");
	  }
	  catch(Exception ex){
		  throw new EJBException(ex);
	  }
...
//<comment description="setter/getters">
	...
	{
		if( COD_CTL_SAN == newCOD_CTL_SAN ) return;
	...
	{
		if(DAT_CRE_CTL_SAN!=null) if( DAT_CRE_CTL_SAN.equals(newDAT_CRE_CTL_SAN)) return;
	...
	{
		if(NOM_MED_RSP_CTL_SAN!=null) if( NOM_MED_RSP_CTL_SAN.equals(newNOM_MED_RSP_CTL_SAN)) return;
	...
	public void removeCOD_DOC(long newCOD_DOC){
		...
		try
		{
			...
			ps.setLong	(4, COD_AZL);
			if(ps.executeUpdate()==0) throw new NoSuchEntityException("Riga con ID="+newCOD_DOC+" non e' trovata");
		}
		catch(Exception ex)
		{
			throw new EJBException(ex);
	...
	public void removeCOD_PRO_SAN(long newCOD_PRO_SAN){
		...
		try
		{
			...
			ps.setLong	(3, COD_CTL_SAN);
			ps.setLong	(4, COD_AZL);
			if(ps.executeUpdate()==0) throw new NoSuchEntityException("Riga con ID="+newCOD_PRO_SAN+" non e' trovata");
		}
		catch(Exception ex)
		{
			throw new EJBException(ex);
	...
	public void removeCOD_VST_IDO(long newCOD_VST_IDO,long COD_DPD, long COD_CTL_SAN,long COD_AZL,java.sql.Date DATA_PIF_VST_IDO){
		...
		try
		{
         ...
         if(ps.executeUpdate()==0) throw new NoSuchEntityException("Riga con ID="+newCOD_VST_IDO+" non e' trovata");
		}
		catch(Exception ex)
		{
			throw new EJBException(ex);
	...
	public void removeCOD_VST_MED(long newCOD_VST_MED,long COD_DPD, long COD_CTL_SAN,long COD_AZL,java.sql.Date DATA_PIF_VST_MED){
		...
		try
		{
			...
			if(ps.executeUpdate()==0) throw new NoSuchEntityException("Riga con ID="+newCOD_VST_MED+" non e' trovata");
		}
		catch(Exception ex)
		{
			throw new EJBException(ex);
...
PseudoContext.bind("CartelleSanitarieBean", new CartelleSanitarieBean());
////////////////////////////////////////// </BINDING> /////////////////////////////////////////////////////////////////
%>
