...
<%-- 
    Document   : APP_ANA_RIS_Delete
    Created on : 28-mag-2008, 15.24.53
...
<%
    IAppAnalisiRischi AppAnalisiRischi = null;
    String strCOD_RSO = request.getParameter("ID");
    if ( strCOD_RSO != null) {
        IAppAnalisiRischiHome home = (IAppAnalisiRischiHome) 
                PseudoContext.lookup("AppAnalisiRischiBean");
        try {
            long lCOD_SRV = Long.parseLong(request.getParameter("ID_PARENT"));
            long lCOD_RSO = Long.parseLong(strCOD_RSO);
            home.remove(new AppAnalisiRischiPK(lCOD_SRV,lCOD_RSO));
        } catch (Exception ex) {
